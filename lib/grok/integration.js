// ============================================================
// GROK INTEGRATION LAYER - Conscientiousness Edition v2.2
// ============================================================
...
const CONFIG = {
  ...
  INTENT_MAP: {
    ...
    synthesis: 'synthesis',
    cot: 'cotContextual',
    framework: 'professionalFramework',
    professional: 'professionalFramework'
  },
...
export class GrokIntegration {
  ...
  constructor(apiKey, options = {}) {
    ...
    this.client = new GrokClient(apiKey, {
      ...
      enableFrameworkMode: options.enableFrameworkMode !== undefined ? 
        options.enableFrameworkMode : true,
      enableStructuredOutput: options.enableStructuredOutput !== undefined ? 
        options.enableStructuredOutput : true,
      extractSections: options.extractSections !== undefined ? 
        options.extractSections : true
    });
    ...
    this.defaultFrameworkMode = options.defaultFrameworkMode || CONFIG.DEFAULTS.FRAMEWORK_MODE;
    this.defaultStructuredOutput = options.defaultStructuredOutput || CONFIG.DEFAULTS.STRUCTURED_OUTPUT;
    ...
    this.includeSections = options.includeSections !== undefined ? 
      options.includeSections : CONFIG.FORMATTING.INCLUDE_SECTIONS;
    ...
    console.log('[GrokIntegration] Framework mode: ' + this.defaultFrameworkMode + ', Structured output: ' + this.defaultStructuredOutput);
  }
  ...
  async generateResponse(query, context, options = {}) {
    ...
    try {
      ...
      const frameworkMode = options.frameworkMode !== undefined ? options.frameworkMode : this.defaultFrameworkMode;
      const structuredOutput = options.structuredOutput !== undefined ? options.structuredOutput : this.defaultStructuredOutput;
  ...
  /**
   * Generate a response with the professional framework
   * @param {string} query - User's question
   * @param {string} context - Retrieved context
   * @param {object} options - Additional options
   * @returns {Promise<object>} Framework response
   */
  async generateWithFramework(query, context, options = {}) {
  ...
  _buildPrompt(query, context, intent, personality, options) {
    ...
    // If framework mode is enabled, use the framework builder
    if (options.frameworkMode || options.structuredOutput || intent === 'professionalFramework') {
...
• Use proper formatting with paragraphs and bullet points`;
  ...
  _formatResponse(result, metadata) {
    ...
    var formatted = {
      ...
      metadata: {
        ...
        frameworkMode: metadata.frameworkMode || false,
        structuredOutput: metadata.structuredOutput || false
      }
    };

    // Add framework sections if available
    if (this.includeSections && result.sections) {
      formatted.metadata.sections = result.sections;
      formatted.metadata.framework_sections = true;
    }
  ...
  /**
   * Enable or disable framework mode
   * @param {boolean} enabled - Whether to enable framework mode
   */
  setFrameworkMode(enabled) {
    this.defaultFrameworkMode = enabled;
    console.log('[GrokIntegration] Framework mode set to: ' + enabled);
  }

  /**
   * Enable or disable structured output
   * @param {boolean} enabled - Whether to enable structured output
   */
  setStructuredOutput(enabled) {
    this.defaultStructuredOutput = enabled;
    console.log('[GrokIntegration] Structured output set to: ' + enabled);
  }
}
