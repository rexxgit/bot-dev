// ============================================================
// EXPERT PERSONAS - Conscientiousness Edition v2.0
// ============================================================
...
export function getAvailablePersonas() {
  return Object.keys(expertPersonas).map(id => ({
    id: id,
...
export function getPersonasByType(type) {
  return Object.keys(expertPersonas)
    .filter(id => expertPersonas[id].type === type)
    .map(id => ({
      id: id,
      name: expertPersonas[id].name,
      description: expertPersonas[id].description
...
export function searchPersonas(searchTerm) {
  ...
  return Object.keys(expertPersonas)
    .filter(id => {
      const persona = expertPersonas[id];
...
export function getPersonaTypes() {
  const types = new Set();
  Object.values(expertPersonas).forEach(persona => {
    if (persona.type) {
